...
3. Merge — `.github/workflows/generate-mirrors.yml` regenerates the vendor mirrors (`scripts/check-generated.sh` previews this locally).
...
2. Did it leave `.claude/skills/`, `.agents/skills/`, `.github/skills/` alone? They're regenerated by CI after merge — hand edits there get overwritten.
3. Does every `skills/*/custom/` folder still hold only `.gitkeep` placeholders (`scripts/check-custom-empty.sh`)?
